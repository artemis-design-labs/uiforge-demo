...
- [Mobbin MCP Server](#mobbin-mcp-server)
...
Connect Claude directly to [Mobbin](https://mobbin.com) - the world's largest UI/UX design reference library. This MCP server allows Claude and Claude Code to search apps, browse screens, and analyze user flows from Mobbin's database of 100,000+ mobile and web app screenshots.

**No external downloads or Figma integration required** - authenticates directly with your Mobbin account.

## Features

| Feature | Description |
|---------|-------------|
| **Direct Authentication** | Magic link login via email (no API keys needed) |
| **App Search** | Search apps by name, category, or keywords |
| **Screen Browsing** | Get all UI screens from any app with pattern filtering |
| **Flow Analysis** | View user journey flows (onboarding, checkout, etc.) |
| **Category Browse** | Browse apps by category and platform |
| **Pattern Discovery** | Get available UI patterns and screen types |
| **Token Persistence** | Auth tokens saved locally for seamless reconnection |

## Available Tools

| Tool | Description |
|------|-------------|
| `mobbin_auth_status` | Check if authenticated and get user info |
| `mobbin_login` | Start login (sends verification code to email) |
| `mobbin_verify` | Complete login with verification code |
| `mobbin_search_apps` | Search apps by query, platform, limit |
| `mobbin_get_app_screens` | Get screens for an app with optional pattern filter |
| `mobbin_get_app_flows` | Get user journey flows from an app |
| `mobbin_browse_apps` | Browse apps by platform and category |
| `mobbin_get_categories` | List available app categories |
| `mobbin_get_patterns` | List available UI patterns |
...
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  1. mobbin_login(email)                                          │
│         │                                                        │
│         ▼                                                        │
│  ┌─────────────────┐                                             │
│  │ Supabase Auth   │───▶ Magic link email sent                   │
│  │ /auth/v1/otp    │                                             │
│  └─────────────────┘                                             │
│         │                                                        │
│         ▼                                                        │
...
│         │                                                        │
│         ▼                                                        │
│  4. Tokens persisted to ~/.mobbin-mcp/tokens.json                │
│         │                                                        │
│         ▼                                                        │
│  5. Ready to search apps, screens, and flows!                    │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
└── src/
    ...
    ├── auth.ts            # Supabase authentication (magic link flow)
    ├── api.ts             # Mobbin API client (RPC endpoints)
...
| File | Purpose |
|------|---------|
| `/packages/mobbin-mcp/src/index.ts` | MCP server with 9 tool handlers |
| `/packages/mobbin-mcp/src/auth.ts` | Supabase magic link authentication |
| `/packages/mobbin-mcp/src/api.ts` | API client for Mobbin's Supabase backend |
| `/packages/mobbin-mcp/src/types.ts` | TypeScript interfaces (MobbinApp, MobbinScreen, MobbinFlow) |

## Configuration

The Mobbin MCP server is configured in `.mcp.json`:
...
The server interfaces with Mobbin's Supabase backend:

| RPC Function | Purpose |
|--------------|---------|
| `get_apps_with_preview_screens_filter` | Fetch apps with pagination |
| `get_app_screens_filter` | Fetch screens for an app version |
| `get_app_sections_filter` | Fetch user flows for an app version |
| `get_apps_count` | Get total app count by platform |

## Usage Examples

### Check Authentication Status
```
Use mobbin_auth_status to check if I'm logged in
```

### Login to Mobbin
```
Login to Mobbin using my email: user@example.com
```
...
### Get App Screens
```
Show me onboarding screens from the Spotify app
```

### Get User Flows
```
Get the checkout flow from the Airbnb app
```

### Browse by Category
```
Browse Finance apps on Android
```
...
Authentication tokens are persisted to:
```
~/.mobbin-mcp/tokens.json
```

This allows the server to automatically reconnect on restart without requiring re-authentication.

## Limitations

1. **No Official API** - Uses reverse-engineered endpoints from Mobbin's web app
2. **Rate Limits** - Mobbin may impose undocumented rate limits
3. **Authentication Expiry** - Tokens may expire and require re-login
4. **Search is Client-Side** - App search filters results after fetching pages

## Future Enhancements

- [ ] Implement proper rate limiting
- [ ] Add caching layer for frequently accessed data
- [ ] Support for screen image analysis
- [ ] Integration with UI Forge component matching
- [ ] Bulk export of design patterns
